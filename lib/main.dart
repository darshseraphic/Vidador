...
class _MainLayoutRouterState extends ConsumerState<MainLayoutRouter> { //
  ...
  Widget build(BuildContext context) { //
    ...
    return Scaffold( //
      ...
      bottomNavigationBar: Container( //
        ...
        child: SafeArea( //
          child: Row( // Row items are now wrapped cleanly inside Expanded zones
  ...
  Widget _buildChronoTabItem(int index, String uppercaseLabel, Color selectedColor, Color unselectedColor) {
    ...
    // Each element is wrapped inside an Expanded column zone to claim 20% width grid allocation
    return Expanded(
      child: GestureDetector(
        onTap: () => setState(() => _currentMatrixIndex = index), //
        behavior: HitTestBehavior.opaque, // Forces the transparent parts of the column box to intercept touches
        child: Container(
          height: 52, // Generates massive vertical hitting space matching the standard bar layout
          alignment: Alignment.center, // Centers text accurately inside its specific 20% grid division
          child: Text(
